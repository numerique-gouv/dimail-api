...
def test_something(db_api_maker, log):
    ...
    response = client.post("/admin/users", json={"name": "testing", "password": "toto", "is_admin": False})
    assert response.status_code == 200
    ...
    assert response.status_code == 200
    assert response.json() == [{"name": "testing", "uuid": uuid, "is_admin": False}]
    response = client.post("/admin/users", json={"name": "testing", "password": "titi", "is_admin": False})
